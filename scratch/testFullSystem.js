...
const { register, login, googleLogin } = require("../backend/controllers/authController");
...
async function runFullSystemCheck() {
  ...
  try {
    ...
    console.log(`✅ [3/7] Google Sign-In Authentication: VERIFIED (Email: ${googleUser.email})`);
    results.push({ test: "Google Sign-In Authentication", status: "PASS" });
  } catch (err) {
  ...
  try {
    const mockDocketNumber = `CTP-${new Date().getFullYear()}-${Math.floor(100000 + Math.random() * 900000)}`;
    testComplaint = await Complaint.create({
      docketNumber: mockDocketNumber,
      userId: testUser ? testUser._id : new mongoose.Types.ObjectId(),
      citizenName: "Integration Test User",
      citizenEmail: testEmail,
      citizenPhone: "9876543210",
      companyName: "Samsung India",
      category: "Electronics",
      productName: "Galaxy S21 Display",
      issueDescription: "Green line appearing on AMOLED display after software update.",
    ...
    console.log(`✅ [4/7] Grievance Filing & Docket Generation: SUCCESS (Docket: ${testComplaint.docketNumber})`);
  ...
  try {
    const fetched = await Complaint.findOne({ docketNumber: testComplaint.docketNumber });
    if (fetched && fetched.companyName === "Samsung India") {
      console.log(`✅ [5/7] Public Docket Tracking & Speed Post Verification: VERIFIED (Status: ${fetched.status})`);
      results.push({ test: "Docket Tracking & Lookup", status: "PASS" });
    } else {
      throw new Error("Could not find created complaint by docket number.");
    }
  } catch (err) {
  ...
  // 6. Test Brand Leaderboard Aggregation
  try {
  ...
  console.log("Database disconnected cleanly. Test finished.");
}
